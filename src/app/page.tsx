...
import { stationNameToId } from "@/lib/station-utils";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export default function Home() {
  ...
  const handleStationVerified = (stationName: string) => {
    ...
    console.log(`Station verified: "${stationName}" -> "${stationId}"`);
    console.log("Current visited stations:", visitedStations);
    if (!visitedStations.includes(stationId)) {
      setVisitedStations((prev) => {
        const newStations = [...prev, stationId];
        console.log("Updated visited stations:", newStations);
        return newStations;
      });
    } else {
      console.log("Station already visited");
    }
  };
        ...
        (error) => {
          switch (error.code) {
            case error.PERMISSION_DENIED:
              setGeolocationError(
                "Geolocation permission denied. Please enable location access."
              );
              break;
  ...
  const sidebarContent = (
    <>
      ...
      <StationVerification
        onStationVerified={handleStationVerified}
              ...
              {geolocationError && (
                <Button variant="outline" size="icon" className="ml-auto">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>{geolocationError}</TooltipContent>
                  </Tooltip>
                </Button>
              )}
              {/* Wallet connection button should go here */}
              {/* Example placeholder: */}
