...
export default function StationVerification({
  ...
  const calculateNearbyStations = useCallback(
    (location: UserLocation) => {
      const stationsWithCoords = allStations
        .map((station) => {
          const coords =
            stationCoordinates[station as keyof typeof stationCoordinates];
      ...
      const nearbyStationsWithDistance = sortedStations
        .slice(0, 10)
        .map((station) => ({
  ...
  const onSubmit = async (data: z.infer<typeof VerificationSchema>) => {
    if (!photoDataUri) {
      toast({
        title: "No Photo",
        description: "Please take a photo to submit.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (result.isAuthentic) {
        ...
        if (photoDataUri) {
          try {
            await saveStationPhoto(
              data.stationName,
              photoDataUri,
              "verification"
            );
            console.log("Photo saved successfully for", data.stationName);
          } catch (error) {
      ...
      <Card className="border-none shadow-none mt-4">
        ...
        <CardContent className="p-0 mt-2">
          <p className="text-sm text-muted-foreground">
            Please connect your wallet to verify station visits and collect
            badges.
          </p>
        </CardContent>
      </Card>
                ...
                onClick={() => {
                  ...
                  if (formData.stationName && formData.username) {
                    ...
                    toast({
                      title: "Missing Info",
                      description:
                        "Please select a station and enter username.",
                      variant: "destructive",
                    });
                  }
                }}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="text-center space-y-2">
              <p className="text-sm text-muted-foreground">
                Make sure you're clearly visible with station signs in the
                background
              </p>
