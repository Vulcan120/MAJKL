...
function getStationLines(stationName: string): { color: string; name: string }[] {
  const lines: { color: string; name: string }[] = [];
  
  Object.entries(londonData).forEach(([key, lineData]) => {
    if (typeof lineData === 'object' && lineData !== null && 'nodes' in lineData) {
...
export function generateStationTokenMetadata(stationName: string): StationTokenMetadata {
...
function generateTokenImage(stationName: string, primaryColor: string, lines: { color: string; name: string }[]): string {
  const width = 300;
  const height = 300;
  ...
  const svg = `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="${centerX - 120}" y="${centerY - 15}" width="240" height="30" fill="${primaryColor}" rx="15" />
      
      <!-- Station name text -->
      <text x="${centerX}" y="${centerY + 5}" text-anchor="middle" fill="white" font-family="Arial, sans-serif" font-size="16" font-weight="bold">
        ${stationName}
      </text>
      
      <!-- Line indicators -->
      ${lines.map((line, index) => `
        <circle cx="${centerX - 60 + (index * 30)}" cy="${centerY + 40}" r="8" fill="${line.color}" stroke="white" stroke-width="2" />
      `).join('')}
      
      <!-- TubeHacks branding -->
      <text x="${centerX}" y="${centerY - 60}" text-anchor="middle" fill="${primaryColor}" font-family="Arial, sans-serif" font-size="12" font-weight="bold">
        TUBEHACKS
      </text>
      
      <!-- Verification badge -->
      <circle cx="${centerX + 80}" cy="${centerY - 80}" r="15" fill="#10b981" />
      <text x="${centerX + 80}" y="${centerY - 75}" text-anchor="middle" fill="white" font-family="Arial, sans-serif" font-size="16" font-weight="bold">
        ✓
      </text>
    </svg>
  `;
...
export function saveStationToken(stationName: string, token: StationToken): void {
  try {
...
export async function mintStationToken(stationName: string): Promise<StationToken> {
  const metadata = generateStationTokenMetadata(stationName);
  
  // For demo purposes, simulate minting
  const mockMint = `${stationName.toLowerCase().replace(/\s+/g, '')}_${Date.now()}`;
...
export async function mintStationTokenOnSolana(
  ...
  throw new Error('Solana minting not yet implemented - using mock minting for demo');
}
